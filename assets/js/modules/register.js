(function () {
    ...
    if (regBtn !== null) {
        ...
        const regForm = Forms.getRegisterForm();
        if (regForm !== null) {
            const nConfirm = 'confirm';
            const nPassword = 'password'
            const password = Widget.querySelector('#' + nPassword);
            const confirm = Widget.querySelector('#' + nConfirm);

            const newPassword = new PasswordInput('vt-none', {
                attrs: {
                    id: nPassword,
                    name: nPassword
                }
            });
            const newConfirm = new PasswordInput('vt-none', {
                attrs: {
                    id: nConfirm,
                    name: nConfirm
                }
            ...
            const pn = password.getNode();
            pn.parentElement.replaceChild(newPassword.getNode(), pn);
            const cn = confirm.getNode();
            cn.parentElement.replaceChild(newConfirm.getNode(), cn);
        }
